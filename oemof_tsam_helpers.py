# slapp/utils/oemof_tsam_helpers.py
...
DATAPACKAGE_DIR = Path(__file__).parent
DATAPACKAGE_ORIGINAL = "dispatch"
DATAPACKAGE_TARGET = "dispatch_tsam"

datapackage_path = DATAPACKAGE_DIR / DATAPACKAGE_ORIGINAL
sequences_original_path = DATAPACKAGE_DIR / DATAPACKAGE_ORIGINAL / "data" / "sequences"
elements_original_path = DATAPACKAGE_DIR / DATAPACKAGE_ORIGINAL / "data" / "elements"

datapackage_target_path=DATAPACKAGE_DIR / DATAPACKAGE_TARGET
periods_path = DATAPACKAGE_DIR / DATAPACKAGE_TARGET / "data" / "periods"
sequences_path = DATAPACKAGE_DIR/ DATAPACKAGE_TARGET / "data" / "sequences"
tsam_path = DATAPACKAGE_DIR / DATAPACKAGE_TARGET / "data" / "tsam"
elements_path = DATAPACKAGE_DIR/ DATAPACKAGE_TARGET / "data" / "elements"
...
def crawl_sequences_data(path: Path = sequences_original_path) -> tuple[pd.DataFrame, dict[str, list]]:
    ...
    for file_name in path.iterdir():
        if file_name.is_file() and file_name.suffix in ".csv":
            profiles = pd.read_csv(file_name, encoding="utf-8", delimiter=";", index_col='timeindex')
...
def copy_and_append_datapackage_json(origin_path: Path = datapackage_path, goal_path: Path = datapackage_target_path) -> None:

    file_path= goal_path / 'datapackage.json'

    # Neue Resource, die hinzugefügt werden soll
    ...
    if not file_path.exists():
        shutil.copy(origin_path / 'datapackage.json', file_path)

        # Bestehende datapackage.json-Datei laden
        with open(file_path, 'r', encoding='utf-8') as file:
            datapackage = json.load(file)

        # Neue Resource zur Liste der Resources hinzufügen
        if 'resources' not in datapackage:
            datapackage['resources'] = []

        datapackage['resources'].append(tsam_resource)
        ...
        warnings.warn("Datapackage.json already exits in goal_path. "
                      "Please make sure the tsam resource is in datapackage.json.", UserWarning)
...
if __name__ == "__main__":
    ...
    copy_elements_data()
    copy_and_append_datapackage_json()
